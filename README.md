...
Un Grafana **k3s** séparé : [https://monitoring.k3s.lpb.ovh](https://monitoring.k3s.lpb.ovh) (NS `monitoring`, Prometheus 7 j / PVC 1 Gio, node-exporter, kube-state-metrics). Dashboard **k3s + GGR Trafic** (CPU/RAM nœud et par namespace, disque, pods, PVC, enregistrement GGR, visites par page `/` et `/trafic/<id>`, carte des origines). `./scripts/update.sh` ne déploie pas ce NS.
